...
// Sorted list of all phone codes (unique, for the phone code selector)
export const PHONE_CODES = Array.from(
  new Map(COUNTRIES.map((c) => [c.phoneCode, { code: c.phoneCode, country: c.name, flag: c.code }]))
).map(([, v]) => v).sort((a, b) => a.country.localeCompare(b.country));
